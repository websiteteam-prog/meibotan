...
<?php
// ---------------------------------------------------------------------------
// FRONTEND-ONLY category hiding.
// These categories are still present in the database; we simply skip rendering
// them anywhere in the header (mega menu, mobile drawer, search dropdown).
// To hide another category, add its lowercase name or slug to this list.
// ---------------------------------------------------------------------------
              ...
              <div class="select-custom d-none d-md-block">
                <select id="cat" name="cat">
                  ...
                  foreach ($searchcatdata as $searchcatrow) {
                    if ($isHiddenFrontCategory($searchcatrow)) continue; ?>
    ...
    <div class="container">
      <nav class="main-nav w-100">
        <ul class="menu">
          ...
          // Hide frontend-only categories (e.g. gummies) without removing them from the DB.
          $hparentcatdata = array_values(array_filter($hparentcatdata, function ($c) use ($isHiddenFrontCategory) {
            return !$isHiddenFrontCategory($c);
          }));
...
<?php if (is_file(APPPATH . 'Views/frontend/mobile-search.php')) {
  include(APPPATH . 'Views/frontend/mobile-search.php');
} ?>
